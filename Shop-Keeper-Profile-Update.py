...
for i in rec:
    fn = "./Media/Shopkeeper-Profile/" + i[8]
    print("""
    <h1></h1>
    <h2></h2>
    ...
    <div class="card col-md-7">
        ...
        <br>
        <h4 class="text-center"> Shop Id : %s</h4>
        <h4 class="text-center"> Location : %s , %s</h4>

    """ % (fn, i[2], i[1], i[11],i[12]))
...
if Submit != None:
    if CNew == New:
        q1 = """update shopkeeper set Password="%s" where Password='%s' """ % (New, Pass)
        cur.execute(q1)
        conn.commit()
        print("""
                <script>
...
if Update != None:
    if len(form) != 0:
        ...
        if Profile.filename:
            ...
            open("./Media/Shopkeeper-Profile/" + fl, "wb").write(Profile.file.read())
            q1 = """update shopkeeper set Profile="%s" where Id='%s' """ % (fl, Id)
            cur.execute(q1)
            conn.commit()
            print("""
                <script>
                    alert("profile update success");
                    location.href="./Shop-Keeper-Profile-Update.py?Id=%s";
                </script>
                </body>
                </html>""" % Id)
conn.close()
